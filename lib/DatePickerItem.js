...
class DatePickerItem extends Component{
    ...
    _moveTo(currentIndex) {
        const value = this.state.dates[MIDDLE_INDEX];
        const { disabledDate } = this.props;
        if (disabledDate && disabledDate(value)) return;
    ...
    renderDatepickerItem(date, index) {
        const { disabledDate } = this.props;
        const className =
            (date < this.props.min || date > this.props.max || (disabledDate && disabledDate(date))) ?
            'disabled' : '';
